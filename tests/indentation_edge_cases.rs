use snips::{process_file, get_snippet_diffs};
...
fn indented_source_with_indented_markdown_no_diff() {
    ...
    write_indented_source(&code_path, 
        "fn main() {\n    if true {\n        println!(\"hello\");\n    }\n}\n"
    );
    ...
    write_indented_markdown(&md_path, "    ", "code.rs", 
        "fn main() {\n    if true {\n        println!(\"hello\");\n    }\n}"
    );
    ...
    assert!(render_result.is_none(), "Render should make no changes when content matches");
    assert!(diffs.is_empty(), "Diff should report no changes when content matches");
}
...
fn various_indentation_combinations() {
    ...
    for (name, indent) in test_cases {
        ...
        write_indented_markdown(&md_path, indent, "code.rs", 
            "struct Example {\n    field: i32,\n}\n\nimpl Example {\n    fn method(&self) {\n        println!(\"value: {}\", self.field);\n    }\n}"
        );
        ...
        assert_eq!(render_result1, render_result2, 
...
fn deep_indentation_with_nested_content() {
    ...
    assert_eq!(content_before, content_after, "Content should not change during multiple renders");
    ...
    if render1.is_none() {
        assert!(diffs.is_empty(), "No diffs should be reported when render makes no changes");
    }
}
...
fn empty_lines_and_trailing_whitespace() {
    ...
    if render_result.is_none() {
        assert!(diffs.is_empty(), "No diffs when render makes no changes (empty lines test)");
    }
}
...
fn tabs_vs_spaces_consistency() {
    ...
    write_indented_markdown(&md_path_tabs, "\t", "code.rs", 
        "fn spaces() {\n    println!(\"using spaces\");\n}"
    );
    
    // Test with markdown using spaces  
    let md_path_spaces = dir.path().join("doc_spaces.md");
    write_indented_markdown(&md_path_spaces, "    ", "code.rs", 
        "fn spaces() {\n    println!(\"using spaces\");\n}"
    );
    ...
    for (name, path) in [("tabs", &md_path_tabs), ("spaces", &md_path_spaces)] {
        ...
        if render_result.is_none() {
            assert!(diffs.is_empty(), "No diffs when render makes no changes ({name} test)");
        }
    }
}
...
fn render_diff_consistency_complex() {
    ...
    for i in 1..=5 {
        ...
        if render_result.is_none() {
            assert!(diffs.is_empty(), "Iteration {i}: No diffs when render makes no changes");
        } else {
            assert!(!diffs.is_empty(), "Iteration {i}: Diffs should exist when render makes changes");
        }
    }
}
...
fn named_snippets_with_indentation() {
    ...
    if render_result.is_none() {
        assert!(diffs.is_empty(), "Named snippets: No diffs when render makes no changes");
    } else {
        assert!(!diffs.is_empty(), "Named snippets: Diffs should exist when render makes changes");
    }
}