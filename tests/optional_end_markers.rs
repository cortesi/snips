...
fn write_source_with_optional_end(path: &Path, name: &str, content: &str, include_name_in_end: bool) {
...
fn optional_end_marker_name() {
    ...
    write_source_with_optional_end(&code_path, "example", "fn test() {\n    println!(\"test\");\n}\n", false);
...
fn optional_end_with_indentation() {
    let dir = tempfile::tempdir().unwrap();
    let code_path = dir.path().join("code.rs");
    write_source_with_optional_end(&code_path, "indented-test", "fn indented() {\n    println!(\"indented\");\n}\n", false);
